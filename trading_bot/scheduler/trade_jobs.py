...
# ==========================================================
# 공통 유틸: 주문 실행 + 체결 확인 + 미체결 취소
# ==========================================================
async def _execute_and_verify(broker, ticker, side, qty, price, order_type,
                               today_str, max_attempts=3):
    """
    지정가 주문을 전송하고 체결 여부를 확인합니다.
    미체결 시 취소 후 재시도합니다.

    Returns:
        (success: bool, total_filled: int, last_price: float)
    """
    rem_qty = qty
    last_price = price
    ...
    for attempt in range(max_attempts):
        if rem_qty <= 0:
            return True, qty, last_price

        # 실시간 호가 갱신
        if side == "BUY":
            live_price = await asyncio.to_thread(broker.get_ask_price, ticker)
        else:
            live_price = await asyncio.to_thread(broker.get_bid_price, ticker)
        exec_price = live_price if live_price > 0 else price

        res = broker.send_order(ticker, side, rem_qty, exec_price, order_type)
        odno = res.get('odno', '')

        if res.get('rt_cd') != '0' or not odno:
        ...
        for _ in range(4):
            ...
            unfilled = await asyncio.to_thread(broker.get_unfilled_orders_detail, ticker)
            ...
            execs = await asyncio.to_thread(broker.get_execution_history, ticker, today_str, today_str)
        ...
        if ccld_qty >= rem_qty:
            ...
            if order_found and my_order:
                ord_qty = int(float(my_order.get('ord_qty', 0)))
                rem_qty = ord_qty - ccld_qty
            else:
                rem_qty -= ccld_qty

            await asyncio.to_thread(broker.cancel_order, ticker, odno)
            await asyncio.sleep(1.0)

        await asyncio.sleep(0.5)

    total_filled = qty - rem_qty
    return False, total_filled, last_price


def _get_market_times(now_est):
    """NYSE 장 시간 정보를 반환. 실패 시 기본값 사용."""
    try:
        ...
        if now_est.weekday() < 5:
            return (
                now_est.replace(hour=9, minute=30, second=0, microsecond=0),
                now_est.replace(hour=16, minute=0, second=0, microsecond=0)
            )
        return None


async def _throttled_alert(context, chat_id, cache_dict, cache_key, cooldown, msg):
    """쿨타임 기반 텔레그램 알림 (중복 방지)"""
    now_ts = time.time()
    if now_ts - cache_dict.get(cache_key, 0) > cooldown:
        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')
        cache_dict[cache_key] = now_ts
...
async def scheduled_sniper_monitor(context):
    ...
    times = _get_market_times(now_est)
    if times is None: return
    market_open, market_close = times
    ...
    is_sniper_active_time = now_est >= switch_time
    ...
    async def _do_sniper():
        async with tx_lock:
            ...
            for t in cfg.get_active_tickers():
                ...
                if version == "V17":
                    ...
                    if is_sniper_active_time and not master_switch_alerted.get(t, False):
                        ...
                        msg = (f"📡 <b>[{t}] 마스터 스위치 락온 (10:20 EST)</b>\n"
                               f"▫️ 공포 가중치: {market_weight:.2f}배\n"
                               f"▫️ 고정 타격선(1년 ATR): -{abs(sniper_pct):.2f}%\n"
                               f"▫️ 당일 자율제어: {state_msg}\n"
                               f"👀 오프닝 휩소가 진정되었습니다. 누적된 최고점(${tracking_info['day_high']:.2f})을 앵커로 실사냥을 개시합니다.")
                        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')

                    if not is_sniper_active_time:
                        continue

                    # 하방 매수 로직 (market_weight <= 1.0)
                    if market_weight <= 1.0:
                        if not lock_buy:
                            await _process_downward_sniper(
                                context, app_data, cfg, broker, strategy,
                                t, qty, avg_price, curr_p, prev_c,
                                sniper_pct, tracking_info, allocated_cash,
                                is_regular_session, vwap_status, today_est_str, chat_id
                            )
                    else:
                        pass
                ...
                # V17이면서 market_weight <= 1.0이면 상방 스킵
                if version == "V17":
                    cached_data = target_cache.get(t, {})
                ...
                if target_pct_val:
                    ...
                    if not lock_sell and curr_p >= target_price:
                        await _process_jackpot_sell(
                            context, app_data, cfg, broker, strategy,
                            t, qty, avg_price, curr_p, prev_c,
                            target_price, allocated_cash, vwap_status,
                            today_est_str, chat_id
                        )
                        continue
                ...
                if not lock_sell:
                    await _process_trailing_sell(
                        context, app_data, cfg, broker, strategy,
                        t, qty, q_qty, avg_price, curr_p, prev_c,
                        base_trigger, star_price, split, t_val, is_rev,
                        tracking_info, allocated_cash, vwap_status,
                        is_regular_session, today_est_str, chat_id
                    )
...
# 하방 매수 스나이퍼 (추출)
# ==========================================================
async def _process_downward_sniper(context, app_data, cfg, broker, strategy,
                                     t, qty, avg_price, curr_p, prev_c,
                                     sniper_pct, tracking_info, allocated_cash,
                                     is_regular_session, vwap_status, today_est_str, chat_id):
    ...
    if not tracking_info['is_tracking'] and current_drop >= (abs(sniper_pct) / 100.0):
        ...
        if not tracking_info['alerted']:
            msg = (f"🎯 <b>[{t}] 하방 매수 스나이퍼 안전장치 해제 (Armed)!</b>\n"
                   f"📈 <b>장중 고점: ${tracking_info['day_high']:.2f}</b>\n"
                   f"📉 <b>에너지 소진: -{abs(sniper_pct):.2f}% (방어선 ${tracking_info['armed_price']:.2f} 돌파)</b>\n"
                   f"👀 진짜 바닥을 다질 때까지 발목을 노리며 추적을 시작합니다.")
            await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')
            tracking_info['alerted'] = True

    if not tracking_info['is_tracking']:
        return
    ...
    if not (is_yangbong and is_rebounded and is_volume_spike):
        return

    is_deep_oversold = True if c_vwap <= 0 else (c_close < c_vwap)

    if not is_deep_oversold:
        fail_history = app_data.setdefault('sniper_fail_ts', {})
        await _throttled_alert(context, chat_id, fail_history, f"{t}_vwap", 300,
            f"🛡️ <b>[{t}] 하방 스나이퍼 매수 취소 (VWAP 필터 가동)</b>\n"
            f"반등 조건은 충족했으나 현재가(${c_close:.2f})가 당일 평균가(${c_vwap:.2f})를 넘어선 비싼 가격입니다. 가짜 반등(데드캣 바운스)으로 판별하여 사격을 취소합니다.")
        return

    # Strong Down 필터
    if vwap_status.get('is_strong_down', False):
        fail_history = app_data.setdefault('sniper_fail_ts', {})
        await _throttled_alert(context, chat_id, fail_history, f"{t}_fake_bounce", 300,
            f"🛡️ <b>[{t}] 하방 스나이퍼 매수 보류 (Strong Down 필터 가동)</b>\n"
            f"거래량의 50% 이상이 VWAP 아래로 쏠린 강력한 하락 추세장(기관 패닉셀)입니다.\n"
            f"어설픈 5분봉 데드캣 바운스에 속지 않고 더 깊은 바닥까지 관망합니다.")
        return

    if not cfg.get_secret_mode():
        return
    ...
    if sniper_budget < exec_price or exec_price <= 0:
        return
    ...
    hunt_qty = math.floor(sniper_budget / exec_price)
    success, filled, actual_price = await _execute_and_verify(
        broker, t, "BUY", hunt_qty, exec_price, "LIMIT", today_est_str
    )

    if success:
        cfg.set_lock(t, "SNIPER_BUY")
        tracking_info['hit_price'] = actual_price
        tracking_info['is_tracking'] = False
        try:
            with open(f"data/sniper_cache_{t}.json", "w") as f:
                json.dump({"hit_price": actual_price, "lowest_price": tracking_info['lowest_price']}, f)
        except: pass
        ...
        msg = (f"💥 <b>[{t}] 하방(매수) 스나이퍼 명중! ({session_str})</b>\n"
               f"📉 <b>최저점: ${tracking_info['lowest_price']:.2f}</b>\n"
               f"📈 <b>반등 양봉 포착 (+{trigger_pct}% 돌파)</b>\n"
               f"🔥 <b>{vol_str}</b>\n"
               f"🎯 <b>상한선(${tracking_info['armed_price']:.2f}) 방어막 내에서 최적 단가 ${actual_price:.2f}에 즉시 체결을 완료</b>했습니다!\n"
               "🔫 당일 하방 스나이퍼 활동만을 종료하며, 상방(익절) 감시는 계속됩니다.")
        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')
    else:
        fail_history = app_data.setdefault('sniper_fail_ts', {})
        await _throttled_alert(context, chat_id, fail_history, t, 60,
            f"🛡️ <b>[{t}] 능동형 스나이퍼 기습 실패 (1분 쿨타임 진입)</b>\n"
            f"📉 반등 타점(${exec_price:.2f})에 3회 지정가 덫을 던졌으나 잔량이 남았습니다.\n"
            f"🦇 매수 스나이퍼는 1분 동안 숨을 죽이며, 취소했던 방어 매수(LOC) 주문만 호가창에 정밀 복구합니다.")
...
# 잭팟(12%) 상방 익절 스나이퍼 (추출)
# ==========================================================
async def _process_jackpot_sell(context, app_data, cfg, broker, strategy,
                                  t, qty, avg_price, curr_p, prev_c,
                                  target_price, allocated_cash, vwap_status,
                                  today_est_str, chat_id):
    ...
    success, filled, actual_price = await _execute_and_verify(
        broker, t, "SELL", qty, target_price, "LIMIT", today_est_str
    )

    if success:
        cfg.set_lock(t, "SNIPER_SELL")
        msg = (f"🎉 <b>[{t}] 12% 잭팟 대원칙 달성! (전량 강제 익절)</b>\n\n"
               f"▫️ 12% 목표가(${target_price:.2f}) 돌파를 확인했습니다.\n"
               f"▫️ 대기 중인 모든 쿼터/LOC 방어선을 즉시 해제합니다.\n"
               f"▫️ 잔여 보유 수량 전량({qty}주)에 대해 지정가 익절을 집행하여 단가 ${actual_price:.2f}에 100% 수익을 확정했습니다. 🚀\n"
               "\n🔫 당일 상방(매도) 스나이퍼 활동을 완전 종료합니다.")
        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')
    else:
        fail_history = app_data.setdefault('sniper_j_fail_ts', {})
        await _throttled_alert(context, chat_id, fail_history, t, 60,
            f"🛡️ <b>[{t}] 스나이퍼 잭팟 기습 실패 (방어선 복구)</b>\n"
            f"🎯 3회에 걸쳐 전량 익절을 시도했으나 체결되지 않았습니다.\n"
            f"🦇 취소했던 원래의 매도(SELL) 주문을 다시 호가창에 정밀 장전합니다. (1분 쿨타임 적용)")
...
# 상방 트레일링 쿼터 스나이퍼 (추출)
# ==========================================================
async def _process_trailing_sell(context, app_data, cfg, broker, strategy,
                                   t, qty, q_qty, avg_price, curr_p, prev_c,
                                   base_trigger, star_price, split, t_val, is_rev,
                                   tracking_info, allocated_cash, vwap_status,
                                   is_regular_session, today_est_str, chat_id):
    # 트레일링 시작 (락온)
    if curr_p >= base_trigger and not tracking_info['trailing_armed']:
        ...
        msg = (f"🦇 <b>[{t}] 상방 트레일링 스나이퍼 락온 (Armed)!</b>\n\n"
               f"▫️ 현재가(${curr_p:.2f})가 쿼터 기준선(${base_trigger:.2f})을 돌파했습니다.\n"
               f"▫️ 기존 LOC 덫은 유지한 채, 당일 최고가(Day High)를 실시간 갱신합니다.\n"
               f"📈 이익 극대화를 위한 고점 스캔을 시작합니다!")
        await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')
    ...
    if not (curr_p <= drop_trigger or is_vwap_death_cross):
        return
    ...
    if not target_odno:
        fail_history = app_data.setdefault('sniper_sync_fail_ts', {})
        await _throttled_alert(context, chat_id, fail_history, t, 300,
            f"⚠️ <b>[{t}] 상방 스나이퍼 가로채기 대기 (이중 체결 방지)</b>\n"
            f"가격(${curr_p:.2f})이 붕괴 조건을 충족했으나, 한투 서버에서 기존 방어선(LOC)을 찾지 못했습니다.\n"
            "중복 매도를 막기 위해 5분 뒤 다시 식별을 시도합니다.")
        return
    ...
    success, filled, actual_price = await _execute_and_verify(
        broker, t, "SELL", q_qty, intercept_price, "LIMIT", today_est_str
    )
    ...
    if success:
        ...
        msg += (f"🛡️ 익절은 무조건 정답이다!\n"
                f"기본 방어선(${base_trigger:.2f})이 뚫리기 전, 기존 LOC 주문을 취소하고 지정가 ${actual_price:.2f}에 {q_qty}주를 가로채어 수익을 강제 확정했습니다. 🎯\n")
        ...
        fail_history = app_data.setdefault('sniper_q_fail_ts', {})
        await _throttled_alert(context, chat_id, fail_history, t, 60,
            f"🛡️ <b>[{t}] 스나이퍼 쿼터 가로채기 실패 (방어선 복구)</b>\n"
            f"🎯 지정가 기습 매도를 시도했으나 잔량이 남았습니다.\n"
            f"🦇 취소했던 쿼터 방어선(LOC 매도)을 호가창에 정밀 복구합니다. (1분 쿨타임 적용)")
        ...
        async with tx_lock:
            ...
            for p_ticker, p_orders in p_trade_data.items():
                ...
                for o in p_orders:
                    ...
                    if slice_qty > rem_qty: slice_qty = rem_qty
                    if slice_qty <= 0: continue

                    if side == "BUY" and curr_p > target_price: continue
                    if side == "SELL" and curr_p < target_price: continue
            ...
            for t in cfg.get_active_tickers():
                ...
                if vwap_status.get('is_strong_up', False) and not vwap_cache.get(f"{t}_hold_alerted"):
                    ...
                    msg = (f"🛡️ <b>[{t}] VWAP 매수 보류 (Strong Up 필터 가동)</b>\n"
                           f"거래량 지배력이 당일 평균가 위에 집중된 강력한 상승 추세장입니다.\n"
                           f"FOMO(고점 불타기) 방지를 위해 남은 VWAP 매수 예산 집행을 전면 보류(Hold)하며, 하단 종가(LOC) 방어선만 온전히 유지합니다.")
                    await context.bot.send_message(chat_id=chat_id, text=msg, parse_mode='HTML')
                    ...
                    if valid_buy_qty > 0:
                        ...
                        if res.get('rt_cd') == '0' and odno:
                            ...
                            for _ in range(4):
                                ...
                                if my_execs:
                                    ccld_qty = sum(int(float(ex.get('ft_ccld_qty', '0'))) for ex in my_execs)
                                    if ccld_qty >= valid_buy_qty: break
