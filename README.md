...
#### `/start` — 봇 시작
- 운영 스케줄(토큰 갱신, 동기화, 주문 시각) 표시
- 주요 명령어 목록 안내
- 서머타임/겨울시간 자동 감지 정보

#### `/sync` — 통합 지시서
- 실시간 KIS API + 야후 시세 기반 매매 전략 브리핑
- 종목별 T값, 별값, 1회분 예산, 스나이퍼 타격선 표시
- 현재 보유 수량, 평단가, 손익률 실시간 계산
- 인라인 버튼으로 수동 주문 즉시 실행 가능
- 외화 RP 권장 금액 자동 산출

#### `/record` — 장부 동기화
- KIS API 실잔고와 가상 장부를 비교하여 자동 보정 (TrueSync)
- 수량/평단가 오차 감지 시 비파괴 보정(CALIB) 자동 적용
- 보유 종목별 거래 내역 대시보드 렌더링
- 졸업(전량 매도) 자동 감지 → 명예의 전당 박제 + 자동 복리

#### `/history` — 졸업 명예의 전당
- 과거 완료된 매매 사이클(졸업) 기록 조회
- 졸업별 수익금, 수익률, 투자금, 매도금 상세 확인
- 인라인 버튼으로 개별 졸업 거래 내역 펼치기
- 수익 리포트 이미지(background.png 배경) 자동 생성

#### `/version` — 버전 내역
- 코드 업데이트 히스토리를 5개씩 페이징하여 조회
- 인라인 버튼으로 이전/다음 페이지 탐색

### 설정

#### `/settlement` — 분할/복리/버전 설정
- 종목별 분할 수 변경 (20/40/60)
- 목표 수익률(%) 설정 (SOXL 12%, TQQQ 10% 등)
- 복리율(%) 조절 (졸업 수익의 몇 %를 시드에 재투자)
- 매매 엔진 버전 전환 (V14 ↔ V_REV)
- 실시간 변동성 지표(HV/VXN) 연산 결과 표시

#### `/seed` — 시드머니 관리
- 종목별 운용 시드머니 금액 조절
- 추가(+$100), 감소(-$100), 고정(직접 입력) 3가지 방식
- 현재 설정된 시드 금액 실시간 표시

#### `/ticker` — 운용 종목 선택
- SOXL 전용 / TQQQ 전용 / SOXL+TQQQ 통합 운용 모드 전환
- 인라인 버튼으로 원터치 전환

#### `/mode` — 스나이퍼 ON/OFF
- 종목별 상방 익절 스나이퍼 기능을 개별적으로 활성화/비활성화
- 현재 ON/OFF 상태 표시

### 긴급

#### `/reset` — 비상 해제
- 매매 잠금(REG/SNIPER) 강제 해제
- 리버스 모드 강제 탈출
- 장부 초기화 (본장부 + 에스크로 + 큐 삼위일체 소각)
- 인라인 버튼으로 종목 선택 → 확인 단계 필요 (오작동 방지)

#### `/add_q` — V-REV 큐 수동 추가
- V-REV 엔진의 LIFO 큐에 수동으로 매수 기록을 추가
- 양식: `/add_q SOXL 2026-04-06 20 52.16` (종목 날짜 수량 평단가)
- 기존 보유분을 V-REV 큐에 수동 등록할 때 사용
